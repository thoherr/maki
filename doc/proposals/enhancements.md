...
### 15. AI-Assisted Tagging — IMPLEMENTED (v2.0.1 CLI, v2.1.0 Web UI)

> **Status:** Fully implemented. CLI: `dam auto-tag` (v2.0.1). Web UI: "Suggest tags" button on asset detail page and "Auto-tag" batch button in browse toolbar (v2.1.0). See `dam auto-tag --help` and [`proposal-ai-autotagging.md`](proposal-ai-autotagging.md) for details. Uses SigLIP ViT-B/16-256 via ONNX Runtime, feature-gated behind `--features ai`. **Remaining**: multi-model tiers, Ollama integration.
