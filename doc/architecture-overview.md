...
- **Denormalized query columns**: the `assets` table caches `best_variant_hash`, `primary_variant_format`, `variant_count` (and, added later, `face_count`, `leaf_tag_count`, `video_duration`/`video_codec`) computed at write time. This allows the browse grid to join directly to the best variant (one row per asset, no GROUP BY) and display the identity format and variant count without expensive per-query computation. When no variant-level filters are active, the `variants` table is not joined at all.
