...
- **Tag filter**: a chip-based input with autocomplete. Type to see tag suggestions, click or press Enter to add a tag chip. Multiple tags narrow the results (AND logic). Remove a tag by clicking the x on its chip, or press Backspace in an empty input to remove the last chip. Each chip has three inline toggles: `+`/`−` (include vs exclude — a `−` chip is the inline equivalent of `-tag:foo` in the search field), `▼`/`=`/`/` (mode: include descendants vs whole-path vs leaf-only), and `cc`/`Cc` (case-insensitive vs sensitive). Adding, removing, or toggling a chip triggers an immediate search.
...
**Auto-tag** *(Pro)*: analyzes each selected asset with the SigLIP vision model and applies suggested tags above the configured confidence threshold. A confirmation dialog shows the count of selected assets. After processing, a summary reports how many tags were applied. This is the one-shot "trust the model" path — every suggestion above threshold is written immediately.

**Review tags…** *(Pro)*: same model pass as Auto-tag, but instead of writing immediately, opens a two-pane review modal so you can prune the AI's picks before any tag is committed. Operates on the current selection if non-empty, otherwise on every asset matching the live filter. The modal opens immediately in a "Computing…" phase that streams progress while the model loads and scores assets (the first request can take 30–60 s while the model warms up — the progress bar reassures you nothing is hung). When the pass completes, the modal flips to a two-pane review:

- **Left pane**: every tag the model proposed, sorted by candidate count, with a mean-confidence bar per row. Click a tag to inspect its candidates in the right pane. Three states per tag — pending (default), applied (after you commit it, strikethrough on the row), or skipped (dismissed without applying).
- **Right pane**: thumbnail grid of every asset the selected tag was proposed for, pre-checked with a confidence overlay. A `from: <label> →` caption appears when the AI vocabulary mapping is non-identity (e.g. SigLIP scored `skyscraper` but the vocabulary maps that to `subject|urban|architecture|skyscraper`). A confidence slider auto-toggles cells above/below the threshold; a "Skip already-tagged assets" filter hides assets that already carry the tag.
...
The results bar includes an "Export all" link that exports all assets matching the current search and filter state — not just the current page. Clicking it opens the same export modal as the batch Export button. The full browse state (query, type, tag, format, volume, rating, label, collection, path, person, stack collapse, default-filter override) is sent to the server, which resolves the matching assets through the exact same pipeline as the browse grid — so the archive contains precisely the assets the grid shows. With stacks collapsed, only the stack covers are exported; switch the view to expanded stacks first if you want every member.
...
Dry-run and live import share the same flow: the dialog switches to a **live progress view** with a progress bar, running counts of imported/skipped/locations/recipes, and a scrolling log of the most recent files. Progress is streamed via Server-Sent Events, so you can keep the page open and watch in real time. Dry-run shows a "Dry run…" status line instead of "Importing…" but otherwise behaves identically — same SSE stream, same re-attach behaviour, same minimize-to-toast option.

A **pulsing red dot** next to the "Import" entry in the navigation indicates that a job is running anywhere in the catalog. Click the entry while a job is in flight and the dialog **re-attaches to the running job**: it replays the last ~100 events and continues with the live stream, so a page reload during a long import doesn't lose the activity feed. This works for both live imports and dry-runs. Only one import (live or dry) runs at a time, server-side.

When a live import finishes, the dialog shows a summary and a **Browse imported** link that scopes the browse view to the newly imported assets — exact `id:` filter for small batches (up to 80 assets), or the volume + subfolder filter sorted by date for larger ones.

When a dry run finishes, the dialog shows the same summary in "Would import N" form (the "Browse imported" link is hidden since nothing was actually written) plus a **Run for real** button that re-launches with the exact same form parameters as a live import — so you can review the dry-run counts, decide it's what you wanted, and commit in one click. This is the web equivalent of `maki import` for the most common workflow: plug in a card, register it (if needed), dry-run to sanity-check the count, then import.
...
- **Writeback** — flushes pending metadata edits to `.xmp` recipe files on disk. Web equivalent of `maki writeback`. Always works regardless of `[writeback] enabled` (the config flag governs only *automatic* flush on every edit; this dialog is the explicit manual flush). The form exposes:
  ...
  - `Force — re-write every recipe in the explicit scope regardless of pending state` — the `--force` knob. More aggressive than mirror-tags: rebuilds both `dc:subject` and `lr:hierarchicalSubject` blocks from catalog state, discarding any pre-existing XMP entry. Use after a catalog import that may have stale pending flags, or to clean up runaway entity-escape damage (`Bobby &amp;amp;amp;…`-style leftovers) or leaf-only stale entries (`Bavaria`, `Konzert`) that mirror-tags didn't catch. Combinable with `--mirror-tags` is redundant — `--force` is the stricter superset.
