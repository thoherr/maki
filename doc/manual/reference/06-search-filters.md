...
**Negation per chip (`-`):** In the web UI, each tag chip has a `+` / `−` toggle on its left that flips the chip between include (default) and exclude. A negated chip routes into the search's NOT clause — assets carrying that tag are hidden, regardless of the other chips' modes or case settings. The chip is visually marked (red tint, strikethrough on the label) so the polarity is obvious at a glance. This is the chip-UI equivalent of typing `-tag:foo` in the query field, and works alongside the mode and case toggles on the same chip (e.g. a chip can be "exclude `cat` at any depth, case-sensitive" with all three toggles active).
