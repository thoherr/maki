...
**Directory-local grouping**: By default, stem matching is restricted to assets within the same session root. A session root is the deepest directory component whose name matches the `[group] session_root_pattern` regex (default: `^\d{4}-\d{2}`, i.e. directories starting with a date like `2024-10-05-wedding`). Everything below a session root — `Capture/`, `Selects/`, `Output/` and their subdirectories — belongs to the same session. If no component matches, auto-group falls back to the parent directory. This prevents `DSC_0001` from a 2019 shoot being grouped with `DSC_0001` from a 2024 shoot. Use `--global` to match across all directories (dangerous — only for carefully scoped queries). See the [group configuration](08-configuration.md#group-section) for customizing the session root pattern.
...
**Embedding storage**: Image embeddings are stored in the SQLite catalog (`embeddings` table) per model. Switching models does not corrupt existing embeddings; each model's embeddings are stored separately. The `--similar` flag uses stored embeddings from the active model to find visually similar assets via an in-memory index (dot product on L2-normalized vectors). Embeddings are also stored opportunistically by the web UI "Suggest tags", batch "Auto-tag", and "Review tags…" endpoints. Use `maki embed` to batch-generate embeddings without tagging.

**Web UI review flow**: For curated batches where false positives are costly (vacation sets, wedding shoots), the browse-page **Review tags…** button replaces unconditional `--apply` with a per-tag candidate-review UI. MAKI computes suggestions for the assets in scope, then opens a two-pane modal where each suggested tag is reviewed against its candidate set — uncheck false positives, drag the confidence slider, then Apply commits via the existing batch tag endpoint. See [Tagging Guide → Reviewing suggestions](../user-guide/11-tagging-guide.md#reviewing-suggestions-before-committing-web-ui) for the full workflow.
