...
```mermaid
flowchart TD
    main["main.rs (CLI)"] --> asset_service & query & web
    web["web/ (Axum)"] --> asset_service & query

    asset_service --> catalog & metadata_store & content_store & preview
    asset_service --> exif_reader & xmp_reader & device_registry

    query --> catalog & metadata_store & device_registry

    catalog["catalog (SQLite)"]
    metadata_store["metadata_store (YAML)"]
    content_store["content_store (SHA-256)"]
...
Additional modules not shown: `config`, `models/`, `format`, `collection`, `saved_search`, `stack`, `embedded_xmp`, `tag_util`, `shell` — used across layers.

## Module Table

| Module | File(s) | Purpose |
|------------|-------------------|-------------------------------------------------|
| `ai` | `src/ai.rs` | *(Pro, `ai` feature.)* Multi-model SigLIP wrapper with `ModelSpec` registry. Supports SigLIP ViT-B/16-256 (768-dim), SigLIP ViT-L/16-256 (1024-dim), and the multilingual SigLIP 2 base (768-dim) and large (1024-dim) variants. ONNX session management for vision and text encoders, image preprocessing (256×256 squash resize, normalize to [-1,1] CHW tensor), SentencePiece tokenization (pad to 64 tokens), sigmoid scoring (`logit_scale * dot + logit_bias`), and ~100 default photography labels. Provides `ModelSpec`, `get_model_spec()`, `SigLipModel` with `encode_image()`, `encode_texts()`, and `classify()`. |
| `asset_service` | `src/asset_service.rs` | Orchestrates import, export, grouping, auto-grouping, auto-tagging, face detection, VLM description, video metadata extraction, relocate, delete, and preview generation. Coordinates between content store, metadata store, catalog, EXIF/XMP readers, and preview generator. Shared workflow layer used by both CLI handlers and web routes to ensure identical behavior. |
...
| `model_manager` | `src/model_manager.rs` | *(Pro, `ai` feature.)* Download and cache management for SigLIP ONNX model files from HuggingFace. Model-aware: derives HuggingFace URL from `ModelSpec.hf_repo`. Downloads `vision_model_quantized.onnx`, `text_model_quantized.onnx`, and `tokenizer.json` via curl. Provides `ModelManager` with `ensure_model()`, `download_model()`, `remove_model()`, `list_files()`, `total_size()`, and `list_available_models()`. Default cache dir: `~/.maki/models/<model-id>/`. |
| `metadata_store` | `src/metadata_store.rs` | YAML sidecar file read/write operations. Sidecars are the source of truth for asset metadata. Stored at `metadata/<id-prefix>/<id>.yaml` within the catalog root. Handles serialization/deserialization of `Asset` structs with all nested variants, recipes, locations, and tags. |
...
| | `src/models/variant.rs` | `Variant` struct: content_hash, original_filename, format, role, file_size, source_metadata, locations, recipes. Includes `best_preview_index()` / `best_preview_index_details()` for display variant selection, and `compute_best_variant_hash()` / `compute_primary_format()` for denormalized column computation. |
...
| `preview` | `src/preview.rs` | Preview/thumbnail generation. Standard images use the `image` crate (resized to configurable max edge, default 800px). RAW files use `dcraw` or `dcraw_emu` (LibRaw). Videos use `ffmpeg`. Non-visual formats get info cards rendered with `imageproc`/`ab_glyph`. Previews stored at `previews/<hash-prefix>/<hash>.{jpg,webp}`. Smart previews (2560px) stored at `smart_previews/` with the same structure. |
| `query` | `src/query.rs` | Search query parsing and `QueryEngine` orchestration. Parses filter syntax (`tag:X`, `rating:N+`, `label:Red`, `collection:X`, `path:prefix`, `orphan:true`, `missing:true`, `stale:N`, `volume:none`, `similar:<id>[:<limit>]`). `QueryEngine` wraps catalog + metadata store + device registry to provide high-level operations: `search`, `show`, `tag`, `edit`, `set_rating`, `set_description`, `set_name`, `set_color_label`, `auto_group`, `fix_roles`. Includes path normalization for volume-relative prefix matching. |
...
| | `src/web/routes.rs` | All HTTP route handlers: browse, asset detail, tags page, stats page, collections page, stroll page *(Pro)*, search API, asset editing endpoints, batch operations, saved search CRUD, collection CRUD, stroll neighbors API. Filter bar markup extracted into shared Askama includes (`filter_bar.html`, `filter_bar_js.html`). The L2 transitive neighbor feature (fan-out slider on the stroll page) is handled entirely in the frontend JavaScript within `templates/stroll.html`, reusing the existing `/api/stroll/neighbors` endpoint with chained requests — no additional backend route is needed. |
| | `src/web/templates.rs` | Askama HTML templates and template helper structs (`BrowsePage`, `ResultsPartial`, `AssetPage`, `AssetCard`, `TagsPage`, `StatsPage`, `CollectionsPage`, `StrollPage`, `StrollCenter`, `StrollNeighbor`, and HTML fragment types for htmx partial updates). |
| | `src/web/static_assets.rs` | Compile-time embedded static files (`htmx.min.js`, `style.css`) served as in-memory responses. |
| `xmp_reader` | `src/xmp_reader.rs` | XMP sidecar parsing and write-back. Reads XMP files to extract `dc:subject` (tags), `dc:description`, `xmp:Rating`, `xmp:Label`, `dc:creator`, `dc:rights`. Write-back functions: `update_rating()`, `update_label()`, `update_tags()` (operation-level deltas), `update_description()`, `update_hierarchical_subjects()`. Handles XML escaping (`xml_escape` for writes, `xml_unescape` for reads — symmetric inverse pair so multi-pass writebacks don't accumulate `&amp;amp;…` escape layers; `&amp;` decoded LAST in unescape so nested encodings like `&amp;lt;` decode to `&lt;` not `<`), namespace-URI canonicalisation (any prefix bound to the Lightroom namespace — `lr:`, `lightroom:`, or exotic — collapses to one canonical `lr:hierarchicalSubject` block on write), namespace injection, and `rdf:Description` format conversion. The streaming reader uses `quick-xml`'s `Event::Text::unescape()` for entity decoding; the regex-based update paths apply `xml_unescape` at every `<rdf:li>` capture site to keep the in-memory tag set in literal form (matches catalog), preventing the comparison-mismatch bug that drove the runaway-escape behaviour fixed in v4.5.17. |
