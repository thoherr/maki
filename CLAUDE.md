...
- **XMP write-back**: Rating and tag changes made in the DAM (via CLI `dam edit --rating`, `dam tag`, web UI stars, or web UI tag add/remove) are automatically written back to `.xmp` recipe files on disk. Rating uses `xmp_reader::update_rating()` (string-based find/replace of `xmp:Rating`). Tags use `xmp_reader::update_tags()` with operation-level deltas: added tags are inserted into `dc:subject`/`rdf:Bag`, removed tags are deleted — tags added independently in CaptureOne are preserved. If no `dc:subject` block exists, one is injected (with `xmlns:dc` namespace and self-closing `rdf:Description` conversion if needed). Special characters in tags are XML-escaped. After writing, the file is re-hashed via `ContentStore::hash_file()` and the recipe's `content_hash` is updated in both SQLite catalog and YAML sidecar. Offline volumes and missing files are silently skipped with `eprintln` warnings. This enables bidirectional sync with tools like CaptureOne.
